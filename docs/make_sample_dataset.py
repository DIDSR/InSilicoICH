...
from pedsilicoICH.phantoms import (possible_ages,
                                                           load_phantom)
...
desired_counts_reached = False
uid = 0
while not desired_counts_reached:
...
{n_sdh}/{desired_sdh} SDH\n{n_edh}/{desired_edh} EDH')
    ...
    scan_seed = rng.integers(0, 1e6, size=1)[0]
    ...
    try:
        ...
        n_scans = 1
        width = round(scanner.scan_width, ndigits=1)*n_scans
        ...
        if lesion['lesion_type'] == 'round':
            ...
            n_control += 1
        uid += 1
        if (n_control >= desired_control) & (n_iph >= desired_iph) &\
           (n_edh >= desired_edh) & (n_sdh >= desired_sdh):
            desired_counts_reached = True
